...
> **面试速答**
>
> vector 就是动态数组，底层一块连续内存，内部用三个指针（start/finish/end_of_storage）管理已用大小和容量。它解决的是"可变长度的随机访问序列"这个最常见的需求，既要像数组那样缓存友好、能随机访问，又要能动态增长。
>
> 扩容是它的核心机制：容量不够时分配一块新内存（GCC 按 2 倍扩、MSVC 按 1.5 倍扩），把旧元素搬过去、释放旧内存，所以 push_back 是均摊 O(1)。emplace_back 更高效，它通过完美转发在容器内部直接原地构造，省掉了临时对象。
>
> 有几个坑要特别注意：扩容后所有迭代器都失效；即使没扩容，在中间插删后，插入/删除点之后的迭代器也会失效。还有一个恶心的是 `vector<bool>`——它是个特化版本，底层用 bit 存储，不能对元素取引用，实战里一般用 `vector<char>` 替代。
>
> 选型上，vector 是 90% 场景的默认选择。和 deque 比，vector 随机访问更快（真正的连续内存），但头部插删 O(n)；和 array 比，array 是编译期固定大小、零开销。实际用的时候，能预估大小就先 `reserve()`，避免多次扩容拷贝。
...
> **面试速答**
>
> list 是带哨兵节点的环形双向链表，节点散布在堆上，用前后指针串起来。它适用于那种"需要在任意位置频繁插删，而且不能让已有迭代器失效"的场景——因为插删只是改改前后节点的指针，O(1) 搞定，其他迭代器完全不受影响。
>
> 它的 sort 也很有意思：不能用 `std::sort`（那个要求 RandomAccessIterator），必须用成员函数 `list::sort()`，底层是归并排序，只改指针不移动数据。forward_list 是单向链表，每个节点省一个 prev 指针，更省内存。
>
> 优点是插删稳、不需要移动元素；缺点也很明显——节点分散在堆上，缓存非常不友好，每节点还多两个指针的额外开销，而且不支持随机访问。有个坑是 forward_list 连 `size()` 成员都没有，要数元素只能遍历。
>
> 实际工程里，除非是元素很大（移动代价高）、需要频繁中间插删，或者真的严格要求迭代器稳定，否则优先 vector——缓存友好性的碾压效应通常能把链表的理论优势抹平。
...
> **面试速答**
>
> deque 说白了是个分块数组，通过一个中控器（map，即指针数组）管理多个固定大小的连续缓冲区，是一种"伪连续"容器。它存在的意义是：既要首尾都能高效插删，又要保持随机访问能力。
>
> 它的迭代器比较复杂，含 4 个指针（cur/first/last/node）。随机访问虽然是 O(1)，但要先用除法算出在哪个缓冲区、再取模算偏移，常数比 vector 大很多。好处是扩容时只需要新增一个缓冲区挂到中控器上，不用把所有元素拷一遍，扩容很平滑。
>
> 优点是首尾插删都是 O(1)、扩容代价小；缺点是随机访问常数大、有一定内存碎片、迭代器实现复杂。和 vector 对比，vector 只有尾部 O(1)，头部插删要 O(n)——这正是 stack 和 queue 默认底层都用 deque 的原因（queue 需要头部删除）。
>
> 选型很直接：需要双端操作用 deque，只需要尾部操作且追求极致随机访问性能用 vector。
...
> **面试速答**
>
> map/set 是基于红黑树的有序关联容器，元素按 key 自动排序，查找、插入、删除都是 O(log n)。它适用于需要有序存储键值对、按范围查询、或需要自定义排序的场景。
>
> 为什么 STL 选红黑树而不选 AVL？因为 AVL 平衡要求更严格，删除时最坏可能旋转 O(log n) 次；而红黑树删除最多只要 3 次旋转（常数级别），频繁增删场景下红黑树更划算。这是工程上的典型取舍——牺牲一点平衡度换更低的维护成本。
>
> 有个最常踩的坑：map 的 `operator[]` 在 key 不存在时会自动插入一个默认值元素，所以如果你只想"查一下在不在"，必须用 `find()` 或 `at()`，否则容器会莫名其妙变大。
>
> 和 unordered_map 对比，map 是 O(log n) 但支持范围查询和有序遍历，unordered 平均 O(1) 但无序。选型上，需要有序遍历或范围查询就用 map/set，只要快速查找就用 unordered 版本。还有个优势是节点式容器插入不影响其他已有迭代器，这点在需要迭代器稳定的场景很有用。
...
> **面试速答**
>
> unordered_map/set 是基于哈希表的无序关联容器。STL 采用开链法（每个桶挂一条链表）解决冲突，平均 O(1) 的查找和插入，适用于需要最快查找/插入速度、不关心元素顺序的场景。
>
> 它的核心机制是 rehash：当负载因子（元素数/桶数）超过阈值（默认 1.0）时，会重新分配一个更大的桶数组，把所有元素重新散列进去，这次 rehash 本身是 O(n) 的。
>
> 有三种情况会让它退化到 O(n)，要特别注意：哈希函数太差导致大量碰撞、负载因子过高、hash flooding 攻击（恶意构造碰撞数据）。另外自定义类型做 key 时，必须同时提供 hash 函数和 `operator==`，缺一不可。
>
> 和 map/set 对比，unordered 是平均 O(1) vs 有序版的 O(log n)，但代价是最坏 O(n)、不支持有序遍历、rehash 时所有迭代器失效。实际工程里，大多数"查找"场景都是首选它，预估好元素量后先 `reserve()` 避免 rehash；需要有序遍历或范围查询时再回退到 map/set。
...
> **面试速答**
>
> 迭代器失效的本质就是：迭代器指向的元素被移动或释放之后，你还继续用它，就是未定义行为。理解失效规律是写安全的"遍历+修改"代码的前提，否则会出很隐蔽的 UB。
>
> 不同容器的失效规则差别很大。连续内存容器（vector/deque）因为插删会移动元素，所以失效范围大——vector 扩容时全部失效，未扩容时插入/删除点之后的迭代器失效；节点式容器（list/map/set）因为元素在堆上独立存活，只有被删掉的那个节点的迭代器失效；unordered 容器在 rehash 时所有迭代器都会失效。
>
> 有个最常见的坑要特别注意：erase 后千万不要再 `++it`，正确写法是 `it = container.erase(it)`，用 erase 的返回值接住下一个有效迭代器——这是最经典的迭代器失效 bug。
>
> 连续容器失效范围大但缓存友好，节点容器几乎不失效但缓存差——这其实是同一个 trade-off 的两面，选容器时要把失效规则和访问模式一起考虑。
...
> **面试速答**
>
> std::sort 的实现叫 IntroSort（内省排序），它组合了快排、堆排和插入排序三种算法，各取所长。单独一个快排最坏会退化到 O(n²)，IntroSort 的巧妙之处在于既保证最坏 O(n log n)，平均又能拿到快排的性能。
>
> 它的分工是这样：大部分时间用快排（缓存友好、平均最快），递归深度一旦超过 2*log2(n) 就切换到堆排（保证不退化），小规模（≤16 元素）用插入排序（常数最小）。
>
> 和 stable_sort 对比，sort 不保证稳定性但更快（原地），stable_sort 底层是归并排序、能保证稳定但需要额外 O(n) 空间。
>
> 有两个坑要特别注意：sort 要求 RandomAccessIterator，list 根本不能用、必须用成员函数 `list::sort()`；还有自定义比较函数必须满足严格弱序（strict weak ordering），否则直接 UB。
>
> 选型上，不需要稳定性就用 sort，需要稳定就用 stable_sort；只需要前 k 个有序用 partial_sort，只需要第 k 大位置上的元素用 nth_element。
...
> **面试速答**
>
> SGI STL 的两级内存分配器是一套为了缓解"频繁 malloc/free 小内存"带来的碎片和系统调用开销而设计的方案——大块走 malloc，小块走内存池。
>
> 它的分工规则是：大于 128B 的请求走第一级，直接 malloc/free；小于等于 128B 的走第二级，维护 16 个空闲链表，分别管理 8B、16B、24B、……、128B 的内存块，按 8 字节对齐，分配时从对应链表取一块，释放时归还到链表里。
>
> 优点是小对象分配极快、碎片少；缺点是内存只还给空闲链表、不还给 OS，长期运行进程的 RSS 可能会偏高。和现代分配器比，jemalloc、tcmalloc 的思路类似但更完善，有线程本地缓存（TLS）、多核扩展性也更好。
>
> 要特别注意的是，现代 GCC 默认的 allocator 已经改回标准的 new/delete，SGI 两级分配器只是作为可选实现保留。但"内存池 + 空闲链表"这个设计思想在高性能场景（游戏引擎、交易系统、数据库）里依然广泛使用，面试也经常考。
